...
**Cross-architecture (64-bit injector → 32-bit WOW64 target):** an x64 `mhook_inject` can inject an **x86** companion into a WOW64 target; the reverse (32→64) is `E_NOTIMPL` (no clean way to spawn a 64-bit thread from WOW64 — deferred). The whole payload is target-arch: the x86 bootstrap thunk is carried as a checked-in byte blob (`inject_bootstrap_thunk_x86_blob.h`) since ml64 can't assemble the x86 `.asm`, and the remote params use the explicit fixed-width `MHOOK_INJECT_REMOTE_PARAMS_X86` (104 bytes, no x64 unwind tail). `LdrLoadDll` is resolved from the target's **32-bit** ntdll (the WOW64 target maps both; `FindDllBaseInProcess` selects the one whose path contains `SysWOW64`, and the file parsed for the RVA is the injector's own ntdll path with `System32`→`SysWOW64`). The `DELAY_UNTIL_INIT` init check reads the target's **32-bit PEB** (the `ProcessWow64Information` pointer; `Ldr` @ +0x0C, `Initialized` @ +0x04). Constraints (all `MHOOK_INJECT_E_PARAMS`): DllPath+FunctionName form only (no FunctionPointer); companion PE machine must match the target; `IoStatusBlock`/`ApcRoutine` completion unsupported cross-arch (a 32-bit target can't write back into the 64-bit caller — Event/sync do work). Dynamic cross-arch requires `MhookDllPath` (the x86 `mhook.dll`); the x86 `mhook_inject.dll` companion is taken from the same directory.
...
### Cross-arch x86 thunk blob must stay in sync with the `.asm`
`inject_bootstrap_thunk_x86_blob.h` is a checked-in byte copy of the assembled `inject_bootstrap_thunk_x86.asm` (`InjectBootstrapThunkEntry`..`End`, no trailing NOP), embedded into the **x64** build because ml64 can't assemble x86. The thunk is position-independent with **no relocations** (all calls go indirectly through the params block), so the raw bytes work at any address. If you edit the x86 `.asm`, regenerate the blob (assemble with `ml.exe`; `dumpbin /SYMBOLS` gives the byte count at `InjectBootstrapThunkEnd`; `dumpbin "/SECTION:.text$mn" /RAWDATA:BYTES` dumps the bytes). The `MhookInjectTest.CrossArchThunkBlobMatches` test (x86 **static Release** only — Debug's incremental-link ILT aliases the symbol to a `jmp` thunk, and the symbol isn't exported from the DLL) `memcmp`s the blob against the linked thunk and fails on drift.
