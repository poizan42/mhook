...
**Cross-architecture (64-bit injector → 32-bit WOW64 target):** an x64 `mhook_inject` can inject an **x86** companion into a WOW64 target. The whole payload is target-arch: the x86 bootstrap thunk is generated into the x64 build at build time (ml64 can't assemble x86 — see the thunk-generation invariant below), and the remote params use the explicit fixed-width `MHOOK_INJECT_REMOTE_PARAMS_X86` (108 bytes, no x64 unwind tail). `LdrLoadDll` is resolved from the target's **32-bit** ntdll (the WOW64 target maps both; `FindDllBaseInProcess` selects the one whose path contains `SysWOW64`, and the file parsed for the RVA is the injector's own ntdll path with `System32`→`SysWOW64`). The `DELAY_UNTIL_INIT` init check reads the target's **32-bit PEB** (the `ProcessWow64Information` pointer; `Ldr` @ +0x0C, `Initialized` @ +0x04). Constraints (all `MHOOK_INJECT_E_PARAMS`): DllPath+FunctionName form only (no FunctionPointer); companion PE machine must match the target. Dynamic cross-arch requires `MhookDllPath` (the x86 `mhook.dll`); the x86 `mhook_inject.dll` companion is taken from the same directory. **All completion modes work cross-arch** (sync, Event, IoStatusBlock, ApcRoutine): because a 32-bit target can't write back into the 64-bit caller, async `IoStatusBlock`/`ApcRoutine` are delivered by a **caller-side watcher thread** (`InjectWatcherProc` in `mhook_inject.cpp`) — the target signals an internal completion event and stashes its `NTSTATUS` in a small caller-owned slot (`MHOOK_INJECT_REMOTE_PARAMS.StatusSlot`); the watcher reads it (`NtReadVirtualMemory`) and does the IOSB write / Event signal / APC re-queue in-process. The watcher is bounded by `TimeoutMs` (target-death surfaces as `STATUS_IO_TIMEOUT`); a thread-per-injection for now (pooling is a future optimization).

**Cross-architecture (32-bit injector → 64-bit target) via a proxy:** a WOW64 process can't create a 64-bit thread, so the 32-bit `Mhook_Inject` launches a native **x64 proxy executable** (`mhook_inject_proxy.exe`) via `RtlCreateUserProcess` and delegates the injection to it (`InjectViaProxy`, `#ifndef _M_X64`, replacing the old `E_NOTIMPL`). Handover is a shared **section** carrying the documented fixed-width `MHOOK_PROXY_BLOCK` (`src/mhook-inject/inject_proxy_params.h`) — a *stable public wire contract* so third parties can write their own proxy. **`MHOOK_PROXY_VERSION` 1 is currently OPEN**: no stable release has shipped using it yet, so the block layout / command-line format may still be changed freely without bumping the version. Once a release ships with v1, treat the layout as frozen and bump the version for any wire change. The section, a completion event, and an access-preserving duplicate of the target handle are all created **inheritable** and the proxy is launched with `InheritHandles=TRUE`, so it gets them at identical handle values (the section handle rides the command line as the trailing hex token; the event/target handle values live in the block) — this sidesteps any cross-arch virtual-memory write back into the 32-bit caller. The proxy reconstructs an x64 `MHOOK_INJECT_PARAMS` and runs the *ordinary same-arch* `Mhook_Inject`, writes `ResultHr`/`ProxyState=DONE`, then signals the event. Sync mode waits on `{event, proxyProcess}` bounded by `TimeoutMs`; async spawns a caller-side `ProxyWatcherProc` (mirrors `InjectWatcherProc` but reads the result from the mapped section) that delivers IOSB/Event/APC in-process and owns the section/event/handle/proxy lifetimes (it is spawned even for fire-and-forget, so an early return can't unmap the section under the running proxy). **The proxy follows the library's own static/dynamic split** (so the companion requirements mirror same-arch): static-flavour proxy links the static `mhook_inject.lib`+`mhook.lib` (self-contained; x64 companion must export `_internal_Execute`, `MhookDllPath` unused); dynamic-flavour proxy imports the x64 `mhook_inject.dll` (companion = `mhook_inject.dll` loaded into the target; user passes a dynamic-style x64 `DllPath` + `MhookDllPath`). New errors: `MHOOK_INJECT_E_NO_PROXY` (0xA0000006, expected — no proxy deployed) and `MHOOK_INJECT_E_PROXY` (0xA0000007, found but failed/crashed). The proxy projects live in `src/mhook_inject_proxy/` and are **x64-only**: `mhook_inject_proxy_lib.vcxproj` → `mhook_inject_proxy.lib` (the **static**-config consumer deliverable; `MhookInjectProxyMain` + optional `MhookInjectProxyEntry`; `CombineWithNtdllExtra` gated on `'$(Platform)'=='x64'`), and `mhook_inject_proxy.vcxproj` → `mhook_inject_proxy.exe` built in **all four** x64 configs — static configs self-link (single self-contained exe), dynamic configs import the x64 DLLs and copy `mhook_inject.dll`+`mhook.dll` next to the exe to form a self-contained **bundle** (via the shared `CopyProjectReferenceDlls` target, below — the dynamic `libmhook` reference is `LinkLibraryDependencies=false` + `CopyOutputDll=true`, so `mhook.dll` is bundled but not imported). Win32 configs are `Utility` no-ops. **Deployment:** the dynamic bundle is dropped as a `mhook_inject_proxy\` *subdirectory* next to the x86 module (so the x64 DLLs don't collide with the x86 same-named ones — the proxy resolves imports from its own dir); the static proxy is a flat single exe. `ProxyPath`=NULL probes flat `mhook_inject_proxy.exe` then `mhook_inject_proxy\mhook_inject_proxy.exe`. **Build order:** the x64 proxy must exist before the x86 build that uses it — `build.ps1` builds all x64 configs before x86, so the default build satisfies it; the x86 `CrossArch_32to64*` tests `GTEST_SKIP` when the proxy isn't present (and the delayed test uses a longer resume delay to absorb the proxy's launch latency).
...
### Shared build logic (`src/Directory.Build.targets`)

Auto-imported by every project under `src/` (the MSBuild `Directory.Build.targets` convention). It carries the `CopyProjectReferenceDlls` target: after build, it copies the output `.dll` (and `.pdb`) of each `ProjectReference` tagged `<CopyOutputDll>true</CopyOutputDll>` into the consuming project's `$(OutDir)`, to stage a runtime layout next to a binary — the unit-test exe's companion DLLs, and the dynamic `mhook_inject_proxy` bundle's x64 `mhook_inject.dll`/`mhook.dll`. It is a no-op for projects with no such references, and only `.dll` outputs are copied (a static-config reference whose output is a `.lib` is skipped), so the same metadata works unchanged across static/dynamic configs.
